...
const geocodeData = [
    ...
    ["Parkovisko pri OC Kaufland", 48.700414, 21.236763]
]
...
function fetchData() {
    for (const geocodeDatum of geocodeData) {
        addMarker(geocodeDatum[1], geocodeDatum[2], 'blue', geocodeDatum[0])
    }
    // fetchKosice()
    // fetchOkolie()
}
...
function isLive() {
    return (Date.now() > 1604120400000 && Date.now() < 1604268000000)
        || (Date.now() > 1604730000000 && Date.now() < 1604872800000)
}
